...
class grade_export_xlsgd extends grade_export {
    ...
    /// Print names of all the fields
        ...
        $local_displaytype=$this->convert_flat_displaytypes_to_array($this->displaytype);

        foreach ($this->columns as $grade_item) {
	    $gradedisplayname2='';
	    foreach ($local_displaytype as $gradedisplayname => $gradedisplayconst) {
		$gradedisplayname2=$gradedisplayname;
	    }
            $myxls->write_string(0, $pos++, $this->format_column_name($grade_item,false,$gradedisplayname2));
           //adds date name 
            if ($grade_item->itemtype<>"course"){
               $myxls->write_string(0, $pos++, get_string("date").' '.$this->format_column_name($grade_item,true));
           }else{
  ...
  /**
    ...
    public function display_previewgd($require_user_idnumber=false) {
        ...
        foreach ($this->columns as $grade_item) {//error_log('Grade Item: '.print_r($grade_item,true)); error_log('Display type: '.print_r($this->displaytype,true));
                $gradedisplayname2='';
            foreach ($local_displaytype as $gradedisplayname => $gradedisplayconst) { 
                 $gradedisplayname2=$gradedisplayname;
              }
            echo '<th>'.$this->format_column_name($grade_item,false,$gradedisplayname2).'</th>';
            
            
            //add a date column
            if ($grade_item->itemtype<>"course"){
//               foreach ($this->displaytype as $gradedisplayname => $gradedisplayconst) { 
                 echo '<th>'.get_string("date"). ' '.$this->format_column_name($grade_item,true)."</th>";
//	       }
               }else{
                    echo '<th> </th>';
                   }
